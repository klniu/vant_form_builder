...
class TreeSelectViewState extends State<TreeSelectView>
  ...
  ///保存当前展示数据的List
  List<TreeNode> expand = new List();

  ///保存List的下标的List，用来做标记用
  List<String> mark = new List();
  ...
  ///保存原始数据为泛型T
  void _parseTreeNode(TreeNode node, {int depth = 0, String fatherId}) {
    node.depth = depth;
    node.parentId = fatherId;
    list.add(node);

    if (node.children != null) {
      for (var dept in node.children) {
        _parseTreeNode(dept, depth: depth + 1, fatherId: node.id);
      }
    }
  }
